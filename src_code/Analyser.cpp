...
int isWord(string & s)
{
	...
	for (int i = 0; i < s.length(); i++)
	{
		...
		{
			continue;					// if it is a letter then continue searching
		}
		else if((s[i] == ',' || s[i] == '.' || s[i] == '?' || s[i] == '!') && (i == s.length()-1))
		{			
			return 2;					// if it is a punc. char occurs at the end of string then it's ok
		}
		else
		{
...
void readAnalyzeFile(string fileName)
{
	...
	bool stillSentence = true;					// a flag to indicate whether a sequance of words 
	...
	while (!in.eof())
	{
		...
		int type = geSentenceType(text);			// check if this string is an end of sentence

		if (check != 0)						
		{
			wordCount++;						// to count number of words		
			if (type >= 1)						// increament the corresponding sentence type
			{
	...
	in.close();
	charCount--;									// to encounter for the last space i.e. exclude the eof character
